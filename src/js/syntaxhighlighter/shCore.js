...
var SyntaxHighlighter = function() {
	...
	function u(a, b) {
		...
		function(c) {
			for (var d = "",
			h = 0; h < c.length - 1; h++) d += e.config.space;
			return d + " "
		});
		if (b != null) a = v(a,
		function(c) {
	...
	function y(a, b) {
		...
		for (var d = null,
		h = [], g = b.func ? b.func: c; (d = b.regex.exec(a)) != null;) {
			var i = g(d, b);
	...
	function f(a) {
		...
		if (! (!a || !b || p(a, "textarea"))) {
			...
			h = [], g = 0; g < d.length; g++) h.push(d[g].innerText || d[g].textContent);
			h = h.join("\r");
	...
	var e = {
		...
		toolbar: {
			getHtml: function(a) {
				...
				for (var c = '<div class="toolbar">',
				d = e.toolbar.items,
				h = d.list,
				g = 0; g < h.length; g++) c += (d[h[g]].getHtml || b)(a, h[g]);
				c += "</div>";
				return c
			},
	...
	e.HtmlScript = function(a) {
		...
		if (c != null) {
			...
			d.htmlScript == null ? window.alert(e.config.strings.alert + (e.config.strings.brushNotHtmlScript + a)) : h.regexList.push({
				...
				func: function(j) {
					for (var l = j.code,
					m = [], s = d.regexList, o = j.index + j.left.length, F = d.htmlScript, q, G = 0; G < s.length; G++) {
						q = y(l, s[G]);
						b(q, o);
						m = m.concat(q)
					}
	...
	e.Highlighter.prototype = {
		...
		getMatchesHtml: function(a, b) {
			...
			for (var d = 0,
			h = "",
			g = this.getParam("brush", ""), i = 0; i < b.length; i++) {
				var k = b[i],
				j;
